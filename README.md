...
- **`stock_compare`의 추정치:** `OPM추정`·`ROE`는 컨센서스 추정치라 확정 실적과 섞어
  인용하면 안 됩니다. 선행PER이 종목 상세에 없으면 실적표의 추정 연도 PER로 보완하며
  (`*` 표시), 두 출처는 기준 시점이 달라 소수점 단위로 어긋날 수 있습니다.
