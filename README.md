...
- **`stock_compare`의 추정치:** `OPM추정`·`EPS(E)`·`ROE(E)`는 컨센서스 추정치라 확정
  ...
  보완합니다(`*` 표시).
- **적자 추정 종목의 선행PER은 `-`입니다.** 음수 PER은 "10배 이하" 같은 필터를 숫자로는
  통과해 버리기 때문입니다(후행 PER도 적자면 `-`). 추정치가 없는 종목과는 `EPS(E)` 열로
  구분됩니다 — 적자 추정은 음수, 추정치 없음은 `-`.
...
| `FAIL` | 호출이 실패하거나 표가 깨졌다 (선행PER ≠ 현재가 ÷ EPS(E), 적자 추정인데 선행PER이 숫자인 경우 포함) |
