...
| `stock_compare` | 여러 종목의 밸류에이션·수익성 비교 (스크리닝용, 최대 50종목 한 번에) |
...
- **현재가는 20:00까지 움직입니다.** 2026-09-14 KRX 애프터마켓(16:00~20:00 실시간 매매)이
  열린 뒤로 네이버 현재가는 정규장이 끝나도 애프터마켓 체결가로 바뀌고, 20:00 이후엔
  애프터마켓 종가입니다. 정규장 종가와 다를 수 있습니다(2026-09-22 삼성전자: 정규장 276,500원,
  애프터마켓 277,500원). 그래서 `stock_price`·`stock_compare`는 현재가의 기준(정규장 실시간 /
  애프터마켓 실시간 / 장 마감 후 마지막 체결가)과 조회 시각을 함께 적습니다. 전일대비는
  정규장 종가 대비입니다. ETF·ETN은 애프터마켓에서 거래되지 않습니다.
- **`stock_compare`의 추정치:** `OPM추정`·`EPS(E)`·`ROE(E)`는 컨센서스 추정치라 확정
  실적과 섞어 인용하면 안 됩니다. 선행PER은 모든 행에서 **현재가 ÷ EPS(E)**로 직접
  계산합니다 — 실적표의 PER(E)는 현재가가 아닌 배치 시점 값(대개 전일 종가 기준)이라
  그대로 쓰면 틀립니다. 추정EPS가 종목 상세에 없으면 실적표 추정 열의 EPS로
  보완합니다(`*` 표시).
- **`stock_compare`는 한 행의 가격을 하나로 맞춥니다.** 시세를 한 요청으로 받아 모든 행이
  같은 시점 가격이고, 시총·PER·PBR도 그 현재가로 계산합니다(PER = 현재가 ÷ EPS,
  PBR = 현재가 ÷ BPS). 종목 상세의 값은 요청 시점이 달라 애프터마켓 중엔 현재가와
  1~2틱 어긋납니다.
- **적자면 PER·선행PER에 `적자`라고 씁니다.** 음수 PER은 "10배 이하" 같은 필터를 숫자로는
  통과하고, 크기 순서도 뜻이 없습니다(적자가 클수록 0에 가깝습니다). `-`는 값이 없다는
  뜻으로만 씁니다. 적자 규모는 `EPS(E)` 열에서 보면 됩니다.
- **PER 비교 전 후행 실적 반영 확인:** 후행 EPS 갱신 시점은 종목마다 다릅니다. 미반영
  종목의 낮은 PER은 이익 개선이 아니라 갱신 지연일 수 있습니다.
- **조회 실패 종목도 행으로 남깁니다.** 상장폐지·합병 소멸된 코드(예: 091990)는 조용히
  빠지지 않고 `조회 실패`로 표시되므로, 전수 스크리닝에서 누락을 알아챌 수 있습니다.
  50개를 넘기면 앞 50개만 조회하고, 생략한 종목코드를 적어 줍니다.
...
도구가 실제로 쓸 만한 값을 돌려주는지 확인한다. 의존성 없이 돈다.

```bash
python verify.py
```

코드가 꺼내 쓰는 응답 키를 네이버 API 실제 응답과 대조하고, 종목 11개
(대형주·중소형주·적자기업·리츠·외국주)로 호출해 이런 것들을 잡는다.

| 판정 | 의미 |
|------|------|
| `DEAD` | 어느 종목 응답에도 없는 키를 코드가 참조하고 있다 |
| `FAIL` | 호출이 실패하거나 표가 깨졌다 (선행PER ≠ 현재가 ÷ EPS(E), 시총·PER·PBR이 그 행의 현재가 기준이 아님, 적자인데 PER·선행PER이 `적자`가 아님, 현재가 기준 줄이 없음, 생략한 종목코드를 안 적음 포함) |
| `warn` | 일부 종목에서만 비는 항목 — 컨센서스 없는 종목의 추정PER 등 정상일 수 있다. 네이버 세션 값이 바뀌어 가격 기준을 못 읽은 경우도 여기 |

필드명이나 표 컬럼을 건드렸다면 커밋 전에 돌릴 것.

⚠️ 스텁으로 `mcp`/`starlette`를 갈아끼우므로 **진짜 패키지에서만 나는 오류**
(import 실패, 스키마 생성 오류)는 못 잡는다. 배포 전에는 실제 의존성을 깔고
`python server.py`로 띄워 `/` 헬스체크와 `tools/list`까지 확인할 것.
